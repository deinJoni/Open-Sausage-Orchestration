...
async function checkEvents() {
  ...
  try {
    ...
    const blocksToSearch = process.env.BLOCKS ? BigInt(process.env.BLOCKS) : 10000n;
    ...
    if (logs.length === 0) {
      ...
      console.log('\nTips:');
      console.log('  - Try searching more blocks: BLOCKS=100000 bun run scripts/check-events.ts');
