...
async function registerName() {
  ...
  const owner = process.env.OWNER
    ? (process.env.OWNER as `0x${string}`)
    : account.address;
  ...
  try {
    ...
    if (!isAvailable) {
      ...
      console.log(
        "Use: PRIVATE_KEY=0x... ENS_NAME=test.buenaas.eth bun run emit-test-event"
      );
      process.exit(0);
    }
    ...
    console.log(
      `   PRIVATE_KEY=0x... ENS_NAME=${label}.buenaas.eth bun run emit-test-event`
    );
