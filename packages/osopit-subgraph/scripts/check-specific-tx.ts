...
const TX_HASH =
  "0x7ee268d8f80286cb6e7a226bbb0bb93c06feae735abc84633aeb66505ebdecbb";
...
async function checkTransaction() {
  ...
  try {
    ...
    const registryLogs = receipt.logs.filter(
      (l) => l.address.toLowerCase() === REGISTRY
    );
    const resolverLogs = receipt.logs.filter(
      (l) => l.address.toLowerCase() === RESOLVER
    );
    ...
    if (registryLogs.length === 0 && resolverLogs.length === 0) {
      ...
      console.log(
        "Your subgraph won't index it because it's not watching these addresses."
      );
    }
