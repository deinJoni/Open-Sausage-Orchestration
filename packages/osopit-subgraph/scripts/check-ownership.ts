...
async function checkOwnership() {
  ...
  try {
    ...
    try {
      ...
      console.log('⚠️  Could not check if record exists (function may not exist on this contract)');
    }
    ...
    try {
      ...
      if (resolverAddr === '0x0000000000000000000000000000000000000000') {
      ...
      } else if (resolverAddr.toLowerCase() === RESOLVER_ADDRESS.toLowerCase()) {
        ...
        try {
          ...
          console.log(`Supports setText: ${supportsSetText ? '✅ Yes' : '❌ No'}`);
        } catch (e) {
